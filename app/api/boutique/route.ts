...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('❌ Boutique GET error:', { error: error?.message, timestamp: new Date().toISOString() })
    return NextResponse.json({ error: 'Erreur chargement boutique', details: error?.message }, { status: 500 })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (error) throw error
    console.log('✓ Product created:', { creator_id, product_id: data?.id })
    return NextResponse.json({ product: data }, { status: 201 })
  } catch (error: any) {
    console.error('❌ Boutique POST error:', { error: error?.message, timestamp: new Date().toISOString() })
    return NextResponse.json({ error: 'Erreur création produit', details: error?.message }, { status: 500 })
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (error) throw error
    console.log('✓ Product updated:', { product_id: id, creator_id })
    return NextResponse.json({ product: data })
  } catch (error: any) {
    console.error('❌ Boutique PATCH error:', { error: error?.message, timestamp: new Date().toISOString() })
    return NextResponse.json({ error: 'Erreur mise à jour produit', details: error?.message }, { status: 500 })
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (error) throw error
    console.log('✓ Product deleted:', { product_id: id, creator_id: creatorId })
    return NextResponse.json({ success: true })
  } catch (error: any) {
    console.error('❌ Boutique DELETE error:', { error: error?.message, timestamp: new Date().toISOString() })
    return NextResponse.json({ error: 'Erreur suppression produit', details: error?.message }, { status: 500 })
  }
}
