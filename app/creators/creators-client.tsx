      ...
      >
        <div style={{ position: 'relative', width: '100%', height: IMG_H, flexShrink: 0, backgroundColor: 'var(--ev-card-bg2)' }}>
          ...
          {(c as any).profile_boosted_until && new Date((c as any).profile_boosted_until) > new Date() && (
            <span style={{ position: 'absolute', top: 7, right: 7, backgroundColor: 'rgba(245,158,11,0.9)', color: '#fff', fontSize: 9, fontWeight: 700, borderRadius: 4, padding: '2px 8px' }}>Boosté</span>
          )}
        </div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <button type="button" role="switch" aria-checked={availableOnly} aria-label="Disponible" onClick={() => setAvailableOnly(v => !v)} style={{ width: 34, height: 20, borderRadius: 10, backgroundColor: availableOnly ? colors.green.primary : 'var(--border-color)', position: 'relative', transition: 'background 0.2s', flexShrink: 0, border: 'none', cursor: 'pointer', padding: 0 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: availableOnly ? 600 : 400, color: availableOnly ? colors.green.primary : 'var(--text-secondary)', cursor: 'pointer' }} onClick={() => setAvailableOnly(v => !v)}>Disponible</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <button type="button" role="switch" aria-checked={openToCollab} aria-label="Open collabs" onClick={() => setOpenToCollab(v => !v)} style={{ width: 34, height: 20, borderRadius: 10, backgroundColor: openToCollab ? colors.violet.primary : 'var(--border-color)', position: 'relative', transition: 'background 0.2s', flexShrink: 0, border: 'none', cursor: 'pointer', padding: 0 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: openToCollab ? 600 : 400, color: openToCollab ? colors.violet.primary : 'var(--text-secondary)', cursor: 'pointer' }} onClick={() => setOpenToCollab(v => !v)}>Open collabs</span>
              </div>
            </div>
          </div>
        ...
        <main style={{ flex: 1, padding: '32px 48px 0 32px', minWidth: 0 }}>
          ...
          {visible.length > 0 ? (
                      ...
                      <div style={{ position: 'relative', width: '100%', height: 180, flexShrink: 0, backgroundColor: 'var(--bg-secondary)', overflow: 'hidden' }}>
                        ...
                        <div style={{ position: 'absolute', top: 8, right: 8, display: 'flex', gap: 4 }}>
                          {(creator as any).profile_boosted_until && new Date((creator as any).profile_boosted_until) > new Date() && (
                            <span style={{ padding: '2px 7px', borderRadius: 4, backgroundColor: colors.violet.primary, color: '#fff', fontSize: 10, fontWeight: 700 }}>Boosté</span>
                          )}
            ...
            <div style={{ textAlign: 'center', padding: '80px 24px' }}>
              ...
              {hasActiveFilters
                ? <button onClick={resetFilters} style={{ padding: '10px 24px', borderRadius: 6, border: `1.5px solid ${colors.violet.primary}`, backgroundColor: 'transparent', color: colors.violet.primary, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Réinitialiser</button>
                : <Link href="/register" style={{ display: 'inline-block', padding: '10px 24px', borderRadius: 6, backgroundColor: colors.violet.primary, color: '#fff', fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>Rejoindre en tant que créateur</Link>
              }
            </div>
          )}
        </main>
